...
const handler = async (m, { conn, command, text }) => {
  ...
  if (command==='unln' && /^\d+$/.test(text?.trim())) {
    ...
    if (meta) {
      const participant = meta.participants.find(p => p.id === m.quoted.sender || p.id === m.quoted.participant)
      userJid = participant?.id ? participant.id : normalizeJid(m.quoted.sender || m.quoted.participant)
    } else userJid = normalizeJid(m.quoted.sender || m.quoted.participant)
  } else if (m.mentionedJid?.length) {
    const meta = await conn.groupMetadata(m.chat).catch(()=>null)
    if (meta) {
      const participant = meta.participants.find(p => p.id === m.mentionedJid[0])
      userJid = participant?.id ? participant.id : normalizeJid(m.mentionedJid[0])
    } else userJid = normalizeJid(m.mentionedJid[0])
  } else if (text) {
    userJid = normalizeJid(text)
  }
  ...
  if (userJid && !dbUsers[userJid]) {
    ...
    if (m.isGroup) {
      try {
        ...
        if (participant) name = participant.notify || participant.id.split('@')[0]
      } catch {}
    }
...
handler.before = async function(m){
  try{
    ...
    for(const u of m.messageStubParameters||[]){
      const participant = meta.participants.find(p => p.id === normalizeJid(u))
      if(!participant) continue
      const data = dbUsers[participant.id]
